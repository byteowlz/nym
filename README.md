...
- `config`-based configuration loader that creates `$XDG_CONFIG_HOME/nym/config.toml` (or platform equivalents) on first run.
- Environment overrides using the `NYM__*` prefix; e.g. `NYM__LOGGING__LEVEL=debug`.
...
- Default config path: `$XDG_CONFIG_HOME/nym/config.toml` (or `%APPDATA%\nym\config.toml` on Windows). Override with `--config <path>`.
- Sample configuration with inline comments is available at `examples/config.toml`.
- Data and state directories default to `$XDG_DATA_HOME/nym` and `$XDG_STATE_HOME/nym` (falling back to `~/.local/share` and `~/.local/state` when unset). Override inside the config file.
...
- Generate completions for your shell:
  ...
  ```bash
  cargo run -- completions bash > target/nym.bash
  ```

...
trx create "Subtask" --parent <epic-id> --json  # Hierarchical subtask (gets ID like epic-id.1)
```
...
6. **Commit together**: Always commit the `.trx/issues.jsonl` file together with the code changes so issue state stays in sync with code state

### Auto-Sync

trx automatically syncs with git:

- Exports to `.trx/issues.jsonl` after changes (5s debounce)
...
Run `trx onboard` to get the content, or see step 2 of the onboard instructions.

### MCP Server (Recommended)

If using Claude or MCP-compatible clients, install the trx MCP server:
...
Then use `mcp__trx__*` functions instead of CLI commands.
...
Run `trx <command> --help` to see all available flags for any command.
For example: `trx create --help` shows `--parent`, `--deps`, `--assignee`, etc.
...
- Run `trx <cmd> --help` to discover available flags
